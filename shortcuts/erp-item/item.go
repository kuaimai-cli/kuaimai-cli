...
const (
	...
	// 与浏览器 POST /item/stock/queryList 表单字段对齐（ARCHIVE_V2 ERP 商品档案库存页）
...
// Register attaches erp-items-core product archive commands to root.
func Register(root *cobra.Command) {
	cmd := &cobra.Command{
		Use:   "erp-item",
		Short: "ERP 商品档案命令（erp-items-core /item，不能直接用于上货）",
	}
...
func listCmd(shortcut bool) *cobra.Command {
	use := "list"
	short := "ERP 商品档案库存列表（POST /item/stock/queryList，form 表单）"
	if shortcut {
		use = "+list"
		short = "ERP 商品档案库存列表（快捷命令）"
	}
	var bodyJSON string
	c := &cobra.Command{
		Use:   use,
...
func countCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "count",
		Short: "ERP 商品档案库存总数（POST /item/stock/queryCount，form 表单）",
		RunE: func(cmd *cobra.Command, args []string) error {
			body, err := common.ParseBodyJSON(bodyJSON)
			if err != nil {
				return err
...
func getDetailCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "get-detail",
		Short: "ERP 商品档案详情（GET /item/getItemDetail）",
		RunE: func(cmd *cobra.Command, args []string) error {
			q := url.Values{}
...
func saveCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "save",
		Short: "保存/修改 ERP 商品档案（POST /item/saveItem）",
		RunE: func(cmd *cobra.Command, args []string) error {
			body, err := common.ParseBodyJSON(bodyJSON)
			if err != nil {
				return err
