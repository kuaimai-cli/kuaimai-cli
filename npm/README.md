...
任意命令结束后（1 小时缓存；已发现新版本会持续提示到升级）若有新版会在 **stderr** 提示；`upgrade` 默认会执行 `npm install -g @kuaimai-cli/cli@latest` 并同步 Skills。仅检查请加 `--check-only`。禁用提示：`KUAIMAI_CLI_SKIP_UPDATE_CHECK=1`。
